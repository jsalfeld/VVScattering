...
export theRND=file_Run2016F_$RANDOM;
...
export msg="hadd -f $SKIMTEMPDIR/data_Run2016F.root "
for datasetName in `cat $theRND.txt`; do
  if [ "$(ls -A $SKIMTEMPDIR/$datasetName)" ] && [[ $datasetName != *"SinglePhoton"* ]]; then
    ...
    hadd -f $SKIMTEMPDIR/photon_Run2016F.root $SKIMTEMPDIR/$datasetName/*.root;
    root -l -q -b MitAnalysisRunII/skimming/80x/makeGoodRunSample.C+\(\"$SKIMTEMPDIR/photon_Run2016F.root\",\"$GOODRUNDIR/photon_Run2016F.root\",\"$JSONFILE\"\);
    mv $GOODRUNDIR/photon_Run2016F.root $SKIMDIR/photon_Run2016F.root;
  else
...
if [ $ALEASTONEPD -eq 1 ]; then
  ...
  root -l -q -b MitAnalysisRunII/skimming/80x/makeGoodRunSample.C+\(\"$SKIMTEMPDIR/data_Run2016F.root\",\"$GOODRUNDIR/data_Run2016F.root\",\"$JSONFILE\"\);  
  mv $GOODRUNDIR/data_Run2016F.root $SKIMDIR/data_Run2016F.root;

  root -l -q -b MitAnalysisRunII/skimming/80x/makeOneSkimSample.C+\(\"$SKIMDIR/data_Run2016F.root\",\"$SKIMDIR/met_data_Run2016F.root\",\"data\",2\);
  else
