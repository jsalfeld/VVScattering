...
for datasetName in `cat $theRND.txt`; do
  if [ "$(ls -A $SKIMTEMPDIR/$datasetName)" ] && [[ $datasetName != *"SinglePhoton"* ]]; then
    ...
    root -l -q -b MitAnalysisRunII/skimming/80x/makeGoodRunSample.C+\(\"$SKIMTEMPDIR/photon_Run2016C.root\",\"$GOODRUNDIR/photon_Run2016C.root\",\"$JSONFILE\"\);
...
if [ $ALEASTONEPD -eq 1 ]; then
  ...
  root -l -q -b MitAnalysisRunII/skimming/80x/makeGoodRunSample.C+\(\"$SKIMTEMPDIR/data_Run2016C.root\",\"$GOODRUNDIR/data_Run2016C.root\",\"$JSONFILE\"\);  
  mv $GOODRUNDIR/data_Run2016C.root $SKIMDIR/data_Run2016C.root;

  root -l -q -b MitAnalysisRunII/skimming/80x/makeOneSkimSample.C+\(\"$SKIMDIR/data_Run2016C.root\",\"$SKIMDIR/met_data_Run2016C.root\",\"data\",2\);
  else
