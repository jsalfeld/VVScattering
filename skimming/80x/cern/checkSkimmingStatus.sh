...
do
  ...
  do
    ...
    diff comp_inp_${PD}_${ERA}.txt comp_out_${PD}_${ERA}.txt|grep "<"|awk '{split($2,a,"NeroNtuples_");printf("./MitAnalysisRunII/bin/80x/cern/skim_batch.sh %s/%s %s/%s %s NeroNtuples_%s data\n",ENVIRON["INPUTDIR"],ENVIRON["thePD"],ENVIRON["SKIMDIR"],ENVIRON["thePD"],a[1],a[2])}' > diff_${PD}_${ERA}.sh;
