...
for datasetName in `cat ${theRND}.txt`; do
  ...
  for fileName in `cat /eos/${SKIMDIR}/${datasetName}.txt`; do
    counter=$((counter+1));
    bsub -q 1nh -o ${workDir}/${SKIMDIR}/${datasetName}/${fileName}.out -J ${SKIMDIR}_${datasetName}_${fileName} $CMSSW_BASE/src/VVScattering/skimming/80x/cern/skim_batch.sh ${INPUTDIR} ${SKIMDIR} ${datasetName} $fileName $TYPE
    #$CMSSW_BASE/src/VVScattering/skimming/80x/cern/skim_batch.sh ${INPUTDIR} ${SKIMDIR} ${datasetName} $fileName $TYPE
