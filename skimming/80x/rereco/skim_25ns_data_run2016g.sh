...
export theRND=file_Run2016G_$RANDOM;
...
export msg="hadd -f $SKIMTEMPDIR/data_Run2016G.root "
for datasetName in `cat $theRND.txt`; do
  if [ "$(ls -A $SKIMTEMPDIR/$datasetName)" ] && [[ $datasetName != *"SinglePhoton"* ]]; then
    ...
    hadd -f $SKIMTEMPDIR/photon_Run2016G.root $SKIMTEMPDIR/$datasetName/*.root;
    root -l -q -b VVScattering/skimming/80x/makeGoodRunSample.C+\(\"$SKIMTEMPDIR/photon_Run2016G.root\",\"$GOODRUNDIR/photon_Run2016G.root\",\"$JSONFILE\"\);
    mv $GOODRUNDIR/photon_Run2016G.root $SKIMDIR/photon_Run2016G.root;
  else
...
if [ $ALEASTONEPD -eq 1 ]; then
  ...
  root -l -q -b VVScattering/skimming/80x/makeGoodRunSample.C+\(\"$SKIMTEMPDIR/data_Run2016G.root\",\"$GOODRUNDIR/data_Run2016G.root\",\"$JSONFILE\"\);  
  mv $GOODRUNDIR/data_Run2016G.root $SKIMDIR/data_Run2016G.root;

  root -l -q -b VVScattering/skimming/80x/makeOneSkimSample.C+\(\"$SKIMDIR/data_Run2016G.root\",\"$SKIMDIR/met_data_Run2016G.root\",\"data\",2\);
  else
