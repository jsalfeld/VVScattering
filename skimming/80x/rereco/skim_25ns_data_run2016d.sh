...
for datasetName in `cat $theRND.txt`; do
  if [ "$(ls -A $SKIMTEMPDIR/$datasetName)" ] && [[ $datasetName != *"SinglePhoton"* ]]; then
    ...
    root -l -q -b VVScattering/skimming/80x/makeGoodRunSample.C+\(\"$SKIMTEMPDIR/photon_Run2016D.root\",\"$GOODRUNDIR/photon_Run2016D.root\",\"$JSONFILE\"\);
...
if [ $ALEASTONEPD -eq 1 ]; then
  ...
  root -l -q -b VVScattering/skimming/80x/makeGoodRunSample.C+\(\"$SKIMTEMPDIR/data_Run2016D.root\",\"$GOODRUNDIR/data_Run2016D.root\",\"$JSONFILE\"\);  
  mv $GOODRUNDIR/data_Run2016D.root $SKIMDIR/data_Run2016D.root;

  root -l -q -b VVScattering/skimming/80x/makeOneSkimSample.C+\(\"$SKIMDIR/data_Run2016D.root\",\"$SKIMDIR/met_data_Run2016D.root\",\"data\",2\);
  else
