...
for datasetName in `cat $theRND.txt`; do
  if [ "$(ls -A $SKIMTEMPDIR/$datasetName)" ] && [[ $datasetName != *"SinglePhoton"* ]]; then
    ...
    root -l -q -b VVScattering/skimming/80x/makeGoodRunSample.C+\(\"$SKIMTEMPDIR/photon_Run2016F.root\",\"$GOODRUNDIR/photon_Run2016F.root\",\"$JSONFILE\"\);
...
if [ $ALEASTONEPD -eq 1 ]; then
  ...
  root -l -q -b VVScattering/skimming/80x/makeGoodRunSample.C+\(\"$SKIMTEMPDIR/data_Run2016F.root\",\"$GOODRUNDIR/data_Run2016F.root\",\"$JSONFILE\"\);  
  mv $GOODRUNDIR/data_Run2016F.root $SKIMDIR/data_Run2016F.root;

  root -l -q -b VVScattering/skimming/80x/makeOneSkimSample.C+\(\"$SKIMDIR/data_Run2016F.root\",\"$SKIMDIR/met_data_Run2016F.root\",\"data\",2\);
  else
