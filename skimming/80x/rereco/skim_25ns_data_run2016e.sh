...
export theRND=file_Run2016E_$RANDOM;
...
export msg="hadd -f $SKIMTEMPDIR/data_Run2016E.root "
for datasetName in `cat $theRND.txt`; do
  if [ "$(ls -A $SKIMTEMPDIR/$datasetName)" ] && [[ $datasetName != *"SinglePhoton"* ]]; then
    ...
    hadd -f $SKIMTEMPDIR/photon_Run2016E.root $SKIMTEMPDIR/$datasetName/*.root;
    root -l -q -b MitAnalysisRunII/skimming/80x/makeGoodRunSample.C+\(\"$SKIMTEMPDIR/photon_Run2016E.root\",\"$GOODRUNDIR/photon_Run2016E.root\",\"$JSONFILE\"\);
    mv $GOODRUNDIR/photon_Run2016E.root $SKIMDIR/photon_Run2016E.root;
  else
...
if [ $ALEASTONEPD -eq 1 ]; then
  ...
  root -l -q -b MitAnalysisRunII/skimming/80x/makeGoodRunSample.C+\(\"$SKIMTEMPDIR/data_Run2016E.root\",\"$GOODRUNDIR/data_Run2016E.root\",\"$JSONFILE\"\);  
  mv $GOODRUNDIR/data_Run2016E.root $SKIMDIR/data_Run2016E.root;

  root -l -q -b MitAnalysisRunII/skimming/80x/makeOneSkimSample.C+\(\"$SKIMDIR/data_Run2016E.root\",\"$SKIMDIR/met_data_Run2016E.root\",\"data\",2\);
  else
