...
for datasetName in `cat $theRND.txt`; do
  if [ "$(ls -A $SKIMTEMPDIR/$datasetName)" ] && [[ $datasetName != *"SinglePhoton"* ]]; then
    ...
    root -l -q -b VVScattering/skimming/80x/makeGoodRunSample.C+\(\"$SKIMTEMPDIR/photon_Run2016E.root\",\"$GOODRUNDIR/photon_Run2016E.root\",\"$JSONFILE\"\);
...
if [ $ALEASTONEPD -eq 1 ]; then
  ...
  root -l -q -b VVScattering/skimming/80x/makeGoodRunSample.C+\(\"$SKIMTEMPDIR/data_Run2016E.root\",\"$GOODRUNDIR/data_Run2016E.root\",\"$JSONFILE\"\);  
  mv $GOODRUNDIR/data_Run2016E.root $SKIMDIR/data_Run2016E.root;

  root -l -q -b VVScattering/skimming/80x/makeOneSkimSample.C+\(\"$SKIMDIR/data_Run2016E.root\",\"$SKIMDIR/met_data_Run2016E.root\",\"data\",2\);
  else
