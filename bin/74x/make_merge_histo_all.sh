...
#ls $INPUTDIR|grep -v AODSIM|grep AOD|grep 50ns|awk '{printf("root -l -q -b %s/VVScattering/macros/makeGoodRunSample.C+\\(\\\"%s/%s\\\",\\\"%s/%s\\\",\\\"%s/VVScattering/json/Cert_246908-255031_13TeV_PromptReco_Collisions15_50ns_JSON_v2.txt\\\"\\)\n",ENVIRON["ANADIR"],ENVIRON["INPUTDIR"],$1,ENVIRON["GOODRUNDIR"],$1,ENVIRON["ANADIR"])}' > kkk0;
...
#ls $GOODRUNDIR|grep AOD|awk '{printf("root -l -q -b %s/VVScattering/macros/makeSkimSample.C+\\(\\\"%s/%s\\\",\\\"%s/%s\\\",\\\"data\\\",%d\\)\n",ENVIRON["ANADIR"],ENVIRON["GOODRUNDIR"],$1,ENVIRON["OUTPUTDIR"],$1,ENVIRON["ANADIR"],ENVIRON["FILTER"])}' > kkk1;
