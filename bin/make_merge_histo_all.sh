...
ls $INPUTDIR|grep -v AODSIM|grep AOD|grep 25ns|awk '{printf("root -l -q -b %s/MitAnalysisRunII/macros/makeGoodRunSample.C+\\(\\\"%s/%s\\\",\\\"%s/%s\\\",\\\"%s/MitAnalysisRunII/json/Cert_246908-255031_13TeV_PromptReco_Collisions15_25ns_JSON_v2.txt\\\"\\)\n",ENVIRON["ANADIR"],ENVIRON["INPUTDIR"],$1,ENVIRON["GOODRUNDIR"],$1,ENVIRON["ANADIR"])}' >> kkk0;
