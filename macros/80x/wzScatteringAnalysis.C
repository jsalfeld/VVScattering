...
double bin = -9.;
if(jetDetaJJ <= 3.5 && jetDetaJJ > 2.5){
	if(mjjIn>250. && mjjIn<=500.)  bin=0.;
...
if(jetDetaJJ <= 5. && jetDetaJJ > 3.5){
	if(mjjIn>250. && mjjIn<=500.)  bin=5.;
	if(mjjIn>500. && mjjIn<=1000.) bin=6.;
	if(mjjIn>1000. && mjjIn<=1350.) bin=7.;
	if(mjjIn>1350. && mjjIn<=1750.) bin=8.;
	if(mjjIn>1750.)             bin=9.;
}

if( jetDetaJJ > 5.){
	if(mjjIn>250. && mjjIn<=500.)  bin=10.;
	if(mjjIn>500. && mjjIn<=1000.) bin=11.;
	if(mjjIn>1000. && mjjIn<=1350.) bin=12.;
	if(mjjIn>1350. && mjjIn<=1750.) bin=13.;
	if(mjjIn>1750.)             bin=14.;
}

 if (bin==-9.) cout<<"wrong super variable"<<" mjj: "<<mjjIn<<" detaJJ: "<<jetDetaJJ<<endl;
...
void wzScatteringAnalysis(
 ...
 TString outputDirectory = "./"
  ...
  Float_t jetZepp;
  Float_t trainWeight;
  TFile* trainTree = new TFile("trainTree.root","recreate");
  TTree *mvaTREESig= new TTree("mvaTREESig","tree for MVA training");
  TTree *mvaTREEBack= new TTree("mvaTREEBack","tree for MVA training");

  TMVA::Reader *reader = new TMVA::Reader();
  reader->AddVariable( "jetDeltaEta", &jetDeltaEta ); 
  // reader->AddVariable( "jetDeltaPhi", &jetDeltaPhi );
  reader->AddVariable( "jetMJJ", &jetMJJ );
  reader->AddVariable( "jetOneEta", &jetOneEta );
  reader->AddVariable( "jetTwoEta", &jetTwoEta );
  //reader->AddVariable( "jetOnePhi", &jetOnePt );
  //reader->AddVariable( "jetTwoPhi", &jetTwoPt );
  reader->AddVariable( "jetOnePt", &jetOnePt );
  reader->AddVariable( "jetTwoPt", &jetTwoPt );
    
   
      
   

  //reader->BookMVA( "BDT::BDTG",  "TMVA-v4.2.0/test/weights/TMVAClassificationWZLessVar_BDTG.weights.xml"   );

  mvaTREESig->Branch("new_v", &new_v, "new_v/F");
  mvaTREESig->Branch("jetOnePt", &jetOnePt, "jetOnePt/F");
  mvaTREESig->Branch("jetTwoPt", &jetTwoPt, "jetTwoPt/F");
  mvaTREESig->Branch("jetOneEta", &jetOneEta, "jetOneEta/F");
  mvaTREESig->Branch("jetTwoEta", &jetTwoEta, "jetTwoEta/F");
  mvaTREESig->Branch("jetOnePhi", &jetOnePhi, "jetOnePhi/F");
  mvaTREESig->Branch("jetTwoPhi", &jetTwoPhi, "jetTwoPhi/F");
  mvaTREESig->Branch("jetDeltaEta", &jetDeltaEta, "jetDeltaEta/F");
  mvaTREESig->Branch("jetDeltaPhi", &jetDeltaPhi, "jetDeltaPhi/F");
  mvaTREESig->Branch("jetMJJ", &jetMJJ, "jetMJJ/F");
  mvaTREESig->Branch("jetZepp", &jetZepp, "jetZepp/F");
  mvaTREESig->Branch("trainWeight", &trainWeight, "trainWeight/F");
  mvaTREEBack->Branch("new_v", &new_v, "new_v/F");
  mvaTREEBack->Branch("jetOnePt", &jetOnePt, "jetOnePt/F");
  mvaTREEBack->Branch("jetTwoPt", &jetTwoPt, "jetTwoPt/F");
  mvaTREEBack->Branch("jetOneEta", &jetOneEta, "jetOneEta/F");
  mvaTREEBack->Branch("jetTwoEta", &jetTwoEta, "jetTwoEta/F");
  mvaTREEBack->Branch("jetOnePhi", &jetOnePhi, "jetOnePhi/F");
  mvaTREEBack->Branch("jetTwoPhi", &jetTwoPhi, "jetTwoPhi/F");
  mvaTREEBack->Branch("jetDeltaEta", &jetDeltaEta, "jetDeltaEta/F");
  mvaTREEBack->Branch("jetDeltaPhi", &jetDeltaPhi, "jetDeltaPhi/F");
  mvaTREEBack->Branch("jetMJJ", &jetMJJ, "jetMJJ/F");
  mvaTREEBack->Branch("jetZepp", &jetZepp, "jetZepp/F");
  mvaTREEBack->Branch("trainWeight", &trainWeight, "trainWeight/F");
  //*******************************************************
  //Input Files
  //*******************************************************
...
infilenamev.push_back(Form("%sTT_TuneCUETP8M1_13TeV-powheg-pythia8+RunIIFall15DR76-PU25nsData2015v1_76X_mcRun2_asymptotic_v12_ext3-v1+AODSIM.root",filesPathMC.Data()));infilecatv.push_back(1);
  ...
  //infilenamev.push_back(Form("%sTTZToQQ_TuneCUETP8M1_13TeV-amcatnlo-pythia8.root",filesPathMC.Data()));
  //infilenamev.push_back(Form("%sTTWJetsToQQ_TuneCUETP8M1_13TeV-amcatnloFXFX-madspin-pythia8.root",filesPathMC.Data()));	      infilecatv.push_back(5);			      infilecatv.push_back(5);
   //   infilenamev.push_back(Form("%stZq_ll_4f_13TeV-amcatnlo-pythia8.root",filesPathMC.Data()));                                  infilecatv.push_back(5);  
  // infilenamev.push_back(Form("%sdata_Run2016H.root",filesPathDA.Data())); infilecatv.push_back(0);
...
//infilenamev.push_back("/eos/cms/store/user/jsalfeld/WllMoriond17/met_WLLJJ_WToLNu_EWK_TuneCUETP8M1_13TeV_madgraph-madspin-pythia8.root");  infilecatv.push_back(6);
   ...
   //infilenamev.push_back(Form("%sdata_Run2016B.root",filesPathDA.Data())); infilecatv.push_back(0);   
   /*
  infilenamev.push_back(Form("%sWWW_4F_TuneCUETP8M1_13TeV-amcatnlo-pythia8.root",filesPathMC.Data())); 			      infilecatv.push_back(3);
  infilenamev.push_back(Form("%sWWZ_TuneCUETP8M1_13TeV-amcatnlo-pythia8.root",filesPathMC.Data())); 			      infilecatv.push_back(3);
  infilenamev.push_back(Form("%sWZZ_TuneCUETP8M1_13TeV-amcatnlo-pythia8.root",filesPathMC.Data()));                           infilecatv.push_back(3);
  infilenamev.push_back(Form("%sZZZ_TuneCUETP8M1_13TeV-amcatnlo-pythia8.root",filesPathMC.Data()));                           infilecatv.push_back(3);  
  ...
  infilenamev.push_back(Form("%sTTZToQQ_TuneCUETP8M1_13TeV-amcatnlo-pythia8.root",filesPathMC.Data()));			      infilecatv.push_back(5);
  infilenamev.push_back(Form("%sTTGJets_TuneCUETP8M1_13TeV-amcatnloFXFX-madspin-pythia8.root",filesPathMC.Data()));           infilecatv.push_back(5);
  infilenamev.push_back(Form("%stZq_ll_4f_13TeV-amcatnlo-pythia8.root",filesPathMC.Data()));                                  infilecatv.push_back(2);  
...
cout<<"hello3"<<endl;  
 ...
 TString theVeryTightSFName = "VVScattering/data/80x/veryTightSF_37ifb.root";
  ...
  for(int thePlot=0; thePlot<allPlots; thePlot++){
    ...
    else if(thePlot >= 21 && thePlot <= 21) {nBinPlot = 100; xminPlot = -1.0; xmaxPlot = 1.0;}
    ...
    else if(thePlot >= 46 && thePlot <= 46) {nBinPlot =   10;   xminPlot = 0.0; xmaxPlot =   2.5;}
    ...
    else if(thePlot >= 57 && thePlot <= 57) {nBinPlot =   50;    xminPlot =-1.0; xmaxPlot =   1.0;}
    else if(thePlot >= 58 && thePlot <= 58) {nBinPlot =   50;    xminPlot =-1.0; xmaxPlot =   1.0;}
  ...
  TH1D* histo_Zg_CMS_QCDScaleBoundingUp            = 	new TH1D( "histo_Zg_CMS_QCDScaleBoundingUp"  , "histo_Zg_CMS_QCDScaleBoundingUp"  , nBinMVA, xbins);		histo_Zg_CMS_QCDScaleBoundingUp  ->Sumw2();
  TH1D* histo_Zg_CMS_QCDScaleBoundingDown          = 	new TH1D( "histo_Zg_CMS_QCDScaleBoundingDown"  , "histo_Zg_CMS_QCDScaleBoundingDown"  , nBinMVA, xbins); 	histo_Zg_CMS_QCDScaleBoundingDown  ->Sumw2();
  TH1D* histo_VVV_CMS_QCDScaleBoundingUp           = 	new TH1D( "histo_VVV_CMS_QCDScaleBoundingUp"  , "histo_VVV_CMS_QCDScaleBoundingUp"  , nBinMVA, xbins); 	histo_VVV_CMS_QCDScaleBoundingUp  ->Sumw2();
  TH1D* histo_VVV_CMS_QCDScaleBoundingDown         = 	new TH1D( "histo_VVV_CMS_QCDScaleBoundingDown"  , "histo_VVV_CMS_QCDScaleBoundingDown"  , nBinMVA, xbins); 	histo_VVV_CMS_QCDScaleBoundingDown  ->Sumw2();
  TH1D* histo_WZ_CMS_QCDScaleBoundingUp           = 		new TH1D( "histo_WZ_CMS_QCDScaleBoundingUp"  , "histo_WZ_CMS_QCDScaleBoundingUp"  , nBinMVA, xbins); 		histo_WZ_CMS_QCDScaleBoundingUp  ->Sumw2();
  TH1D* histo_WZ_CMS_QCDScaleBoundingDown           = 	new TH1D( "histo_WZ_CMS_QCDScaleBoundingDown"  , "histo_WZ_CMS_QCDScaleBoundingDown"  , nBinMVA, xbins); 	histo_WZ_CMS_QCDScaleBoundingDown  ->Sumw2();
  TH1D* histo_ZZ_CMS_QCDScaleBoundingUp           = 		new TH1D( "histo_ZZ_CMS_QCDScaleBoundingUp"  , "histo_ZZ_CMS_QCDScaleBoundingUp"  , nBinMVA, xbins); 		histo_ZZ_CMS_QCDScaleBoundingUp  ->Sumw2();
  TH1D* histo_ZZ_CMS_QCDScaleBoundingDown           = 	new TH1D( "histo_ZZ_CMS_QCDScaleBoundingDown"  , "histo_ZZ_CMS_QCDScaleBoundingDown"  , nBinMVA, xbins); 	histo_ZZ_CMS_QCDScaleBoundingDown  ->Sumw2();
  ...
  for(int i = 0; i<11 ; i++)
	{
	  histo_Higgs_CMS_QCDScaleBoundingUp_[i] = new TH1D( "histo_Higgs_"+SignalSuffix[i]+"_CMS_QCDScaleBoundingUp"  , "histo_Higgs_"+SignalSuffix[i]+"_CMS_QCDScaleBoundingUp"  , nBinMVA, xbins); histo_Higgs_CMS_QCDScaleBoundingUp_[i]->Sumw2();//(TH1D*) histoMVA->Clone("histo_Higgs_"+SignalSuffix[i]+"_CMS_QCDScaleBoundingUp");histo_Higgs_CMS_QCDScaleBoundingUp_[i]->Sumw2();	 
	  histo_Higgs_CMS_QCDScaleBoundingDown_[i] = new TH1D( "histo_Higgs_"+SignalSuffix[i]+"_CMS_QCDScaleBoundingDown"  , "histo_Higgs_"+SignalSuffix[i]+"_CMS_QCDScaleBoundingDown"  , nBinMVA, xbins); histo_Higgs_CMS_QCDScaleBoundingDown_[i]->Sumw2();//(TH1D*) histoMVA->Clone("histo_Higgs_"+SignalSuffix[i]+"_CMS_QCDScaleBoundingDown");histo_Higgs_CMS_QCDScaleBoundingDown_[i]->Sumw2();	 
	}
...
TH1D* histo_Higgs_CMS_MVALepEffMBoundingAvg    	 = new TH1D( Form("histo_Higgs_%sAvg",effMName)  , Form("histo_Higgs_%sAvg",effMName)  , nBinMVA, xbins); histo_Higgs_CMS_MVALepEffMBoundingAvg  ->Sumw2();
  ...
  for(UInt_t ifile=0; ifile<infilenamev.size(); ifile++) {
    ...
    //if((infilecatv[ifile] ==0)) numberOfEvents = 50000; 
	...
	histo_WZ_CMS_QCDScaleInitial[5] = histo_WZ_CMS_QCDScaleInitial[5] + TMath::Abs((double)eventMonteCarlo.r5f5);
      ...
      if(i%10000==0) printf("event %d out of %d\n",i,(int)the_input_tree->GetEntries());
      //cout<<"here1"<<endl;
	...
	if(((TLorentzVector*)(*eventJets.p4)[nj])->Pt() > 25 && 
	...
	if(((TLorentzVector*)(*eventJets.p4)[nj])->Pt()* 1.05 >= 30) idJetJESUP.push_back(nj);
     ...
     bool passSystCuts[nSystTypes] = {
	  ...
	  passFilter[5] && ((TLorentzVector*)(*eventMet.metSyst)[BareMet::JesDown])        ->Pt()   > 30 && passFilter[7] && passFilter[8] && passFilter[9] && passFilter[10]
     };
	...
	if(isGenLepton == true) {isGenLep.push_back(1); goodIsGenLep++;}
     ...
     double metRESUP = r.Gaus(met_t,0.1*met_t);
      ...
      bool passVBFJESUP = passAllCuts[0] && idJetJESUP.size() >= 2 &&
                     ( ( *(TLorentzVector*)(eventJets.p4->At(idJetJESUP[0])) )*1.05 + ( *(TLorentzVector*)(eventJets.p4->At(idJetJESUP[1])) )*1.05 ).M() > 500 && TMath::Abs(((TLorentzVector*)(*eventJets.p4)[idJetJESUP[0]])->Eta()-((TLorentzVector*)(*eventJets.p4)[idJetJESUP[1]])->Eta()) > 2.5;
      ...
      bool passVBFLooseMJJLess500 = passAllCuts[0] && idJet.size() >= 2 &&
                     (( ( *(TLorentzVector*)(eventJets.p4->At(idJet[0])) ) + ( *(TLorentzVector*)(eventJets.p4->At(idJet[1])) ) )).M() > 250
			&&
                     (( ( *(TLorentzVector*)(eventJets.p4->At(idJet[0])) ) + ( *(TLorentzVector*)(eventJets.p4->At(idJet[1])) ) )).M() < 500;
     bool passVBFLooseDEtaLess25 = passAllCuts[0] && idJet.size() >= 2 &&
                     (( ( *(TLorentzVector*)(eventJets.p4->At(idJet[0])) ) + ( *(TLorentzVector*)(eventJets.p4->At(idJet[1])) ) )).M() > 250 
			&&  
		      TMath::Abs(((TLorentzVector*)(*eventJets.p4)[idJet[0]])->Eta()-((TLorentzVector*)(*eventJets.p4)[idJet[1]])->Eta()) < 2.5;


      bool passVBFLooseJESUP = passAllCuts[0] && idJetJESUP.size() >= 2 &&
			( ( *(TLorentzVector*)(eventJets.p4->At(idJetJESUP[0])) )*1.05 + ( *(TLorentzVector*)(eventJets.p4->At(idJetJESUP[1])) )*1.05 ).M() > 250 &&
		     TMath::Abs(((TLorentzVector*)(*eventJets.p4)[idJetJESUP[0]])->Eta()-((TLorentzVector*)(*eventJets.p4)[idJetJESUP[1]])->Eta()) > 2.5;

      bool passVBFLooseJESDOWN = passAllCuts[0] && idJetJESDOWN.size() >= 2 &&
			( ( *(TLorentzVector*)(eventJets.p4->At(idJetJESDOWN[0])) )*0.95 + ( *(TLorentzVector*)(eventJets.p4->At(idJetJESDOWN[1])) )*0.95 ).M() > 250 &&
		     TMath::Abs(((TLorentzVector*)(*eventJets.p4)[idJetJESDOWN[0]])->Eta()-((TLorentzVector*)(*eventJets.p4)[idJetJESDOWN[1]])->Eta()) > 2.5;
 
      bool passVBFLooseMETRESUP = passAllCutsMETRESUP && idJet.size() >= 2 &&
                     (( ( *(TLorentzVector*)(eventJets.p4->At(idJet[0])) ) + ( *(TLorentzVector*)(eventJets.p4->At(idJet[1])) ) )).M() > 250 &&
		     TMath::Abs(((TLorentzVector*)(*eventJets.p4)[idJet[0]])->Eta()-((TLorentzVector*)(*eventJets.p4)[idJet[1]])->Eta()) > 2.5;

      bool passVBFLooseMETRESDOWN = passAllCuts[0] && idJet.size() >= 2 &&
                     (( ( *(TLorentzVector*)(eventJets.p4->At(idJet[0])) ) + ( *(TLorentzVector*)(eventJets.p4->At(idJet[1])) ) )).M() > 500 &&
		     TMath::Abs(((TLorentzVector*)(*eventJets.p4)[idJet[0]])->Eta()-((TLorentzVector*)(*eventJets.p4)[idJet[1]])->Eta()) > 2.5;

      bool passVBFLooseLEPScaleUP = passVBFLoose;
      bool passVBFLooseLEPScaleDOWN = passVBFLoose;
     ...
     int theCategory = infilecatv[ifile];
      ...
      new_v=transverseMass2;
      jetOnePt = ((TLorentzVector*)(*eventJets.p4)[idJet[0]])->Pt();
      jetTwoPt = ((TLorentzVector*)(*eventJets.p4)[idJet[1]])->Pt();
      jetTwoEta = ((TLorentzVector*)(*eventJets.p4)[idJet[1]])->Eta();
      jetOneEta = ((TLorentzVector*)(*eventJets.p4)[idJet[0]])->Eta();
      jetTwoPhi = ((TLorentzVector*)(*eventJets.p4)[idJet[1]])->Phi();
      jetOnePhi = ((TLorentzVector*)(*eventJets.p4)[idJet[0]])->Phi();
      jetDeltaEta = (double) TMath::Abs(((TLorentzVector*)(*eventJets.p4)[idJet[0]])->Eta()-((TLorentzVector*)(*eventJets.p4)[idJet[1]])->Eta());
      jetDeltaPhi = (double) TMath::Abs(((TLorentzVector*)(*eventJets.p4)[idJet[0]])->Phi()-((TLorentzVector*)(*eventJets.p4)[idJet[1]])->Phi());
      jetMJJ = (double) (( ( *(TLorentzVector*)(eventJets.p4->At(idJet[0])) ) + ( *(TLorentzVector*)(eventJets.p4->At(idJet[1])) ) )).M();
      jetZepp = 1.;

      trainWeight=totalWeight;

      //double 


      //mjjXdetajj binning
      // double superVariable = superVar(jetMJJ,jetDeltaEta);
      //cout<<superVariable<<endl;
       //superVariable = TMath::Min((( ( *(TLorentzVector*)(eventJets.p4->At(idJet[0])) ) + ( *(TLorentzVector*)(eventJets.p4->At(idJet[1])) ) )).M(),1999.)+2000.*round(jetDeltaEta/1.5);
      //cout<<"rounded: "<<round(jetDeltaEta/1.5)<<" actual values:  "<<jetDeltaEta<<endl;

      
      mvaLoose = 1.;//reader->EvaluateMVA( "BDT::BDTG" );
	...
	double theVar = 0.0;
	bool makePlot = false;
 	if     (thePlot ==  0 && controlSel[0])                          {makePlot = true;theVar = TMath::Min(transverseMass2,1199.999);}
	...
	else if(thePlot == 21 && passVBFLoose)                           {makePlot = true;theVar = TMath::Min(mvaLoose,1.);}
	...
	//else if(thePlot == 40 && passVBFLoose)                            {makePlot = true;theVar = (double)type3l;}
        ...
        else if(thePlot == 46 && pass2JetControl)                          {makePlot = true;theVar = (double) TMath::Min(TMath::Abs(((TLorentzVector*)(*eventJets.p4)[idJet[0]])->Eta()-((TLorentzVector*)(*eventJets.p4)[idJet[1]])->Eta()),2.49999);}
        ...
        else if(thePlot == 50 && passVBFLoose )                         {makePlot = true;theVar = (double)( (TLorentzVector*)(eventLeptons.p4->At(idLep[0])) )->Eta();}
	else if(thePlot == 51 && passVBFLoose )                         {makePlot = true;theVar = (double)( (TLorentzVector*)(eventLeptons.p4->At(idLep[1])) )->Eta();}
	else if(thePlot == 52 && passVBFLoose )                         {makePlot = true;theVar = (double)( (TLorentzVector*)(eventLeptons.p4->At(idLep[2])) )->Eta();}
	...
	else if(thePlot == 57 && passVBFLooseMJJLess500 )                            {makePlot = true;theVar = (double) TMath::Min(mvaLoose,1.);}
	else if(thePlot == 58 && passVBFLooseDEtaLess25 )                            {makePlot = true;theVar = (double)TMath::Min(mvaLoose,1.);}
        else if(thePlot == 59 && passVBFLoose )                            {makePlot = true;theVar = (double)TMath::Min(superVar((( ( *(TLorentzVector*)(eventJets.p4->At(idJet[0])) ) + ( *(TLorentzVector*)(eventJets.p4->At(idJet[1])) ) )).M(),TMath::Abs(((TLorentzVector*)(*eventJets.p4)[idJet[0]])->Eta()-((TLorentzVector*)(*eventJets.p4)[idJet[1]])->Eta())),15.);}
	if( (makePlot && theCategory<6) || (makePlot && theCategory == 6 && infilenamev[ifile].Contains("WLLJJ")) ) histo[     4][thePlot][theCategory]->Fill(theVar,totalWeight);
	if( (makePlot && theCategory<6) || (makePlot && theCategory == 6 && infilenamev[ifile].Contains("WLLJJ")) ) histo[type3l][thePlot][theCategory]->Fill(theVar,totalWeight);
      }

	// Avoid QCD scale and PDF weights that are anomalous high
      double maxQCDscale = (TMath::Abs((double)eventMonteCarlo.r1f2)+TMath::Abs((double)eventMonteCarlo.r1f5)+TMath::Abs((double)eventMonteCarlo.r2f1)+ TMath::Abs((double)eventMonteCarlo.r2f2)+TMath::Abs((double)eventMonteCarlo.r5f1)+TMath::Abs((double)eventMonteCarlo.r5f5))/6.0;

      
      if(1 && (passVBFLoose || passVBFLooseJESDOWN || passVBFLooseJESUP || passVBFLooseMETRESUP || passVBFLooseMETRESDOWN || passVBFLooseLEPScaleUP || passVBFLooseLEPScaleDOWN) && theCategory!=0) {
	//	if(1 && passVBFLoose && theCategory!=0) {


	double MVAVar = -999.;
	double MVAVarMETRESUP = -999.;
	double MVAVarMETRESDOWN = -999.;
	...
	if(passVBFLoose || passVBFLooseMETRESUP || passVBFLooseMETRESDOWN || passVBFLooseLEPScaleUP || passVBFLooseLEPScaleDOWN){
	      MVAVar = TMath::Min( (( ( *(TLorentzVector*)(eventJets.p4->At(idJet[0])) ) + ( *(TLorentzVector*)(eventJets.p4->At(idJet[1])) ) )).M(),1999.);
	      MVAVarMETRESUP = TMath::Min(MVAVar, 1999.);
	      MVAVarMETRESDOWN = TMath::Min(MVAVar, 1999.);
	      MVAVarLEPScaleUP = TMath::Min(MVAVar, 1999.);
	      MVAVarLEPScaleDOWN = TMath::Min(MVAVar, 1999.);}
          	
	if(passVBFLooseJESDOWN){		        
	      MVAVarJESDOWN = TMath::Min(( ( *(TLorentzVector*)(eventJets.p4->At(idJetJESDOWN[0])) ) + ( *(TLorentzVector*)(eventJets.p4->At(idJetJESDOWN[1])) )).M(),1999.);
	}
	if(passVBFLooseJESUP){
	      MVAVarJESUP = TMath::Min(( ( *(TLorentzVector*)(eventJets.p4->At(idJetJESUP[0])) ) + ( *(TLorentzVector*)(eventJets.p4->At(idJetJESUP[1])) )).M(),1999.);
	}

	/////-------------------------        

        if     (theCategory == 0){
	  if(passAllCuts[SIGSEL] && passVBFLoose) {histo_Data->Fill(MVAVar,totalWeight);}
	}

        else if(theCategory == 1){
	  if(passAllCuts[SIGSEL] && passVBFLoose) {
	  ...
	  if(passVBFLooseMETRESUP) histo_Zg_CMS_res_metUp->Fill(MVAVarMETRESUP,totalWeight);
	  if(passVBFLooseMETRESDOWN) histo_Zg_CMS_res_metDown->Fill(MVAVarMETRESDOWN,totalWeight);
	  ...
	  if(passVBFLooseMETRESDOWN) histo_WZ_CMS_res_metDown->Fill(MVAVarMETRESDOWN,totalWeight);
	  if(passVBFLooseMETRESUP) histo_WZ_CMS_res_metUp->Fill(MVAVarMETRESUP,totalWeight);
	  ...
	  if(passVBFLooseMETRESDOWN) histo_ZZ_CMS_res_metDown->Fill(MVAVarMETRESDOWN,totalWeight);
          if(passVBFLooseMETRESUP) histo_ZZ_CMS_res_metUp->Fill(MVAVarMETRESUP,totalWeight);
	  ...
	  if(passVBFLooseMETRESDOWN) histo_VVV_CMS_res_metDown->Fill(MVAVarMETRESDOWN,totalWeight);
	  if(passVBFLooseMETRESUP) histo_VVV_CMS_res_metUp->Fill(MVAVarMETRESUP,totalWeight);
	 ...
	 //cout<<MVAVar<<endl;
	 //cout<<passVBFLoose<<passVBFLooseJESDOWN<<passVBFLooseJESUP<<passVBFLooseMETRESUP<<passVBFLooseMETRESDOWN<<passVBFLooseLEPScaleUP<<passVBFLooseLEPScaleDOWN<<endl;
	  if(passAllCuts[SIGSEL] && passVBFLoose) {
	    if(infilenamev[ifile].Contains("Signal_M700")) Higgs_2d->Fill( (double)((TLorentzVector*)(eventLeptons.p4->At(idLep[0])) )->Pt(), transverseMass2,totalWeight);
	    for(int i=0; i<11; i++) { 
	      if(infilenamev[ifile].Contains("WLLJJ")){
  ...
  for(int thePlot=0; thePlot<allPlots; thePlot++){
    for(int j=0; j<allStates; j++){
      char output[200];
      sprintf(output,outputDirectory+"histowz_nice_%d_%d.root",j,thePlot);	  
...
for(int np=0; np<histBins; np++) {
  ...
  sprintf(outputLimits,outputDirectory+"wz3l%2s_input_%4s.root",finalStateName,ECMsb.Data());
...
histo_ZZ_CMS_QCDScaleBounding[0]->Write();
  histo_ZZ_CMS_QCDScaleBounding[1]->Write();
histo_ZZ_CMS_QCDScaleBounding[2]->Write();
histo_ZZ_CMS_QCDScaleBounding[3]->Write();
histo_ZZ_CMS_QCDScaleBounding[4]->Write();
histo_ZZ_CMS_QCDScaleBounding[5]->Write();
...
//  double lumiE = 1.027;
  ...
  for(int nb=1; nb<=nBinMVA; nb++){
    ...
    sprintf(outputLimitsShape,outputDirectory+"histo_limits_wz3l%2s_%4s_bin%d.txt",finalStateName,ECMsb.Data(),nb-1);
    ...
    newcardShape << Form("shapes * * wz3l3l_input_13TeV2016BBB5.root histo_$PROCESS histo_$PROCESS_$SYSTEMATIC\n");
    newcardShape << Form("shapes data_obs * wz3l3l_input_13TeV2016BBB5.root histo_Data\n");
    newcardShape << Form("shapes Higgs * wz3l3l_input_13TeV2016BBB5.root histo_Higgs_M$MASS histo_Higgs_M$MASS_$SYSTEMATIC\n");
    ...
    newcardShape << Form("%s                               lnN  %7.5f   %7.5f   -   %7.5f   -    %7.5f  \n","lumi_stab_13TeV2016",lumiEstab,lumiEstab,lumiEstab,lumiEstab);	
    newcardShape << Form("norm_WZ_13TeV2016                lnN    -      -    %7.5f    -    -    -  \n",1.14);	
    newcardShape << Form("CMS_eff_b_mistag_13TeV2016       lnN  %7.5f   %7.5f   -   %7.5f   -  %7.5f\n",0.98,0.98,0.98,0.98);
    newcardShape << Form("CMS_wz3l_FakeSys                 lnN    -       -     -     -   %7.5f  -  \n",1.30);
    ...
    newcardShape << Form("%s                                shape     -       -    -      -   %7.5f   -  \n","Fake_2016_bbb_histo_Fake_bin_1",1.);    
    newcardShape << Form("%s                                shape     -       -    -      -   %7.5f   -  \n","Fake_2016_bbb_histo_Fake_bin_2",1.);    
    newcardShape << Form("%s                                shape     -       -    -      -   %7.5f   -  \n","Fake_2016_bbb_histo_Fake_bin_3",1.);    
    newcardShape << Form("%s                                shape     -       -    -      -   %7.5f   -  \n","Fake_2016_bbb_histo_Fake_bin_4",1.);    
    newcardShape << Form("%s                                shape     -       -    -      -   %7.5f   -  \n","Fake_2016_bbb_histo_Fake_bin_5",1.);    
    newcardShape << Form("%s                                shape     -       -    -      -   %7.5f   -  \n","Fake_2016_bbb_histo_Fake_bin_6",1.);
    newcardShape << Form("%s                                shape     -       -    -      -   %7.5f   -  \n","Fake_2016_bbb_histo_Fake_bin_7",1.);
    newcardShape << Form("%s                                shape     -       -    -      -   %7.5f   -  \n","Fake_2016_bbb_histo_Fake_bin_8",1.);
    newcardShape << Form("%s                                shape     -       -    -      -   %7.5f   -  \n","Fake_2016_bbb_histo_Fake_bin_9",1.);
    newcardShape << Form("%s                                shape     -       -    -     %7.5f   -    - \n","ZZ_2016_bbb_histo_ZZ_bin_1",1.);
    newcardShape << Form("%s                                shape     -       -    -     %7.5f   -    - \n","ZZ_2016_bbb_histo_ZZ_bin_2",1.);
    newcardShape << Form("%s                                shape     -       -    -     %7.5f   -    - \n","ZZ_2016_bbb_histo_ZZ_bin_3",1.);
    newcardShape << Form("%s                                shape     -       -    -     %7.5f   -    - \n","ZZ_2016_bbb_histo_ZZ_bin_4",1.);
    newcardShape << Form("%s                                shape     -       -    -     %7.5f   -    - \n","ZZ_2016_bbb_histo_ZZ_bin_5",1.);
    newcardShape << Form("%s                                shape     -       -    -     %7.5f   -    - \n","ZZ_2016_bbb_histo_ZZ_bin_6",1.);
    newcardShape << Form("%s                                shape     -       -    -     %7.5f   -    - \n","ZZ_2016_bbb_histo_ZZ_bin_7",1.);
    newcardShape << Form("%s                                shape     -       -    -     %7.5f   -    - \n","ZZ_2016_bbb_histo_ZZ_bin_8",1.);
    newcardShape << Form("%s                                shape     -       -    -     %7.5f   -    - \n","ZZ_2016_bbb_histo_ZZ_bin_9",1.);
    newcardShape << Form("%s                                shape     -    %7.5f   -       -     -    - \n","VVV_2016_bbb_histo_VVV_bin_1",1.);
    newcardShape << Form("%s                                shape     -    %7.5f   -       -     -    - \n","VVV_2016_bbb_histo_VVV_bin_2",1.);
    newcardShape << Form("%s                                shape     -    %7.5f   -       -     -    - \n","VVV_2016_bbb_histo_VVV_bin_3",1.);
    newcardShape << Form("%s                                shape     -    %7.5f   -       -     -    - \n","VVV_2016_bbb_histo_VVV_bin_4",1.);
    newcardShape << Form("%s                                shape     -    %7.5f   -       -     -    - \n","VVV_2016_bbb_histo_VVV_bin_5",1.);
    newcardShape << Form("%s                                shape     -    %7.5f   -       -     -    - \n","VVV_2016_bbb_histo_VVV_bin_6",1.);
    newcardShape << Form("%s                                shape     -    %7.5f   -       -     -    - \n","VVV_2016_bbb_histo_VVV_bin_7",1.);
    newcardShape << Form("%s                                     shape     -    %7.5f   -       -     -    - \n","VVV_2016_bbb_histo_VVV_bin_8",1.);
    newcardShape << Form("%s                                     shape     -    %7.5f   -       -     -    - \n","VVV_2016_bbb_histo_VVV_bin_9",1.);
    newcardShape << Form("%s                                     shape     -       -   %7.5f    -     -    -  \n","WZ_2016_bbb_histo_WZ_bin_1",1.);
    newcardShape << Form("%s                                     shape     -       -   %7.5f    -     -    -  \n","WZ_2016_bbb_histo_WZ_bin_4",1.);
    newcardShape << Form("%s                                     shape     -       -   %7.5f    -     -    -  \n","WZ_2016_bbb_histo_WZ_bin_5",1.);
    newcardShape << Form("%s                                     shape     -       -   %7.5f    -     -    -  \n","WZ_2016_bbb_histo_WZ_bin_6",1.);
    newcardShape << Form("%s                                     shape     -       -   %7.5f    -     -    -  \n","WZ_2016_bbb_histo_WZ_bin_7",1.);
    newcardShape << Form("%s                                     shape     -       -   %7.5f    -     -    -  \n","WZ_2016_bbb_histo_WZ_bin_8",1.);
    newcardShape << Form("%s                                     shape     -       -   %7.5f    -     -    -  \n","WZ_2016_bbb_histo_WZ_bin_9",1.);
    newcardShape << Form("%s                                     shape   %7.5f       -    -    -     -    -  \n","Zg_2016_bbb_histo_Zg_bin_2",1.);
    newcardShape << Form("%s                                     shape   %7.5f       -    -    -     -    -  \n","Zg_2016_bbb_histo_Zg_bin_1",1.);
    newcardShape << Form("%s                                     shape   %7.5f       -    -    -     -    -  \n","Zg_2016_bbb_histo_Zg_bin_3",1.);
    newcardShape << Form("%s                                     shape   %7.5f       -    -    -     -    -  \n","Zg_2016_bbb_histo_Zg_bin_4",1.);
    newcardShape << Form("%s                                     shape   %7.5f       -    -    -     -    -  \n","Zg_2016_bbb_histo_Zg_bin_5",1.);
    newcardShape << Form("%s                                     shape   %7.5f       -    -    -     -    -  \n","Zg_2016_bbb_histo_Zg_bin_6",1.);
    newcardShape << Form("%s                                     shape   %7.5f       -    -    -     -    -  \n","Zg_2016_bbb_histo_Zg_bin_7",1.);
    newcardShape << Form("%s                                     shape   %7.5f       -    -    -     -    -  \n","Zg_2016_bbb_histo_Zg_bin_8",1.);
    newcardShape << Form("%s                                     shape   %7.5f       -    -    -     -    -  \n","Zg_2016_bbb_histo_Zg_bin_9",1.);
    newcardShape << Form("pdf_qqbar                              lnN  %7.5f      %7.5f  -    %7.5f   -   %7.5f \n",systPDF[0],systPDF[1],systPDF[3],1.0129);
    newcardShape << Form("nuisance edit rename * * CMS_eff_m CMS_eff_m_13TeV2016 \n");
    newcardShape << Form("nuisance edit rename * * CMS_eff_e CMS_eff_e_13TeV2016 \n");
    newcardShape << Form("nuisance edit rename * * CMS_res_met CMS_res_met_13TeV2016 \n");
    newcardShape << Form("nuisance edit rename * * CMS_scale_l CMS_scale_l_13TeV2016 \n");
     //JES shape
    //MET shape
    //LEPES Shape
  ...
  cout<<"histo_WZ "<<float(histo_WZ->Integral()/histo_WZ_CMS_PUBoundingUp->Integral())<<endl;
  cout<<"histo_ZZ "<<float(histo_ZZ->Integral()/histo_ZZ_CMS_PUBoundingUp->Integral())<<endl;
  cout<<"histo_VVV "<<float(histo_VVV->Integral()/histo_VVV_CMS_PUBoundingUp->Integral())<<endl;
  cout<<"histo_Zg "<<float(histo_Zg->Integral()/histo_Zg_CMS_PUBoundingUp->Integral())<<endl;
  cout<<"histo_Higgs "<<float(histo_Higgs->Integral()/histo_Higgs_CMS_PUBoundingUp->Integral())<<endl;



  cout<<"histo_WZ "<<float(histo_WZ->Integral()/histo_WZ_CMS_PUBoundingDown->Integral())<<endl;
  cout<<"histo_ZZ "<<float(histo_ZZ->Integral()/histo_ZZ_CMS_PUBoundingDown->Integral())<<endl;
  cout<<"histo_VVV "<<float(histo_VVV->Integral()/histo_VVV_CMS_PUBoundingDown->Integral())<<endl;
  cout<<"histo_Zg "<<float(histo_Zg->Integral()/histo_Zg_CMS_PUBoundingDown->Integral())<<endl;
  cout<<"histo_Higgs "<<float(histo_Higgs->Integral()/histo_Higgs_CMS_PUBoundingDown->Integral())<<endl;
  


  /*std::cout<<std::endl<<"type3l 0"<<endl;
  for(int i=0; i<eventNumbers0.size(); ++i)
    std::cout <<eventNumbers0[i] << ", ";
  std::cout<<std::endl<<"type3l 1"<<endl;
  for(int i=0; i<eventNumbers1.size(); ++i)
    std::cout <<eventNumbers1[i] << ", ";
  std::cout<<std::cout<<"type3l 2"<<endl;
  for(int i=0; i<eventNumbers2.size(); ++i)
    std::cout <<eventNumbers2[i] << ", ";
  std::cout<<std::endl<<"type3l 3"<<endl;
  for(int i=0; i<eventNumbers3.size(); ++i)
    std::cout <<eventNumbers3[i] << ", ";

  std::cout<<std::endl<<"type3l 0"<<endl;
  for(int i=0; i<runNumbers0.size(); ++i)
    std::cout <<runNumbers0[i] << ", ";
  std::cout<<std::endl<<"type3l 1"<<endl;
  for(int i=0; i<runNumbers1.size(); ++i)
    std::cout <<runNumbers1[i] << ", ";
  std::cout<<std::cout<<"type3l 2"<<endl;
  for(int i=0; i<runNumbers2.size(); ++i)
    std::cout <<runNumbers2[i] << ", ";
  std::cout<<std::endl<<"type3l 3"<<endl;
  for(int i=0; i<runNumbers3.size(); ++i)
    std::cout <<runNumbers3[i] << ", ";

  std::cout<<std::endl<<"type3l 0"<<endl;
  for(int i=0; i<lsNumbers0.size(); ++i)
    std::cout <<lsNumbers0[i] << ", ";
  std::cout<<std::endl<<"type3l 1"<<endl;
  for(int i=0; i<lsNumbers1.size(); ++i)
    std::cout <<lsNumbers1[i] << ", ";
  std::cout<<std::cout<<"type3l 2"<<endl;
  for(int i=0; i<lsNumbers2.size(); ++i)
    std::cout <<lsNumbers2[i] << ", ";
  std::cout<<std::endl<<"type3l 3"<<endl;
  for(int i=0; i<lsNumbers3.size(); ++i)
  std::cout <<lsNumbers3[i] << ", ";*/
