...
BTagEntry::BTagEntry(const std::string &csvLine)
{
  ...
  if (vec.size() != 11) {
std::cerr << "ERROR in BTagCalibration2: "
...
BTagCalibration2::BTagCalibration2(const std::string &taggr):
  tagger_(taggr)
{}
...
void BTagCalibration2::addEntry(const BTagEntry &entry)
{
  data_[token(entry.params)].push_back(entry);
}

const std::vector<BTagEntry>& BTagCalibration2::getEntries(
...
void BTagCalibration2::readCSV(const std::string &s)
{
  std::stringstream buff(s);
  readCSV(buff);
}

void BTagCalibration2::readCSV(std::istream &s)
{
  std::string line;
...
void BTagCalibration2::makeCSV(std::ostream &s) const
{ 
...
std::string BTagCalibration2::makeCSV() const
{
  std::stringstream buff;
  makeCSV(buff);
  return buff.str();
...
std::string BTagCalibration2::token(const BTagEntry::Parameters &par)
{
  std::stringstream buff;
...
BTagCalibration2Reader::BTagCalibration2Reader(const BTagCalibration2* c,
                                             BTagEntry::OperatingPoint op,
...
double BTagCalibration2Reader::eval(BTagEntry::JetFlavor jf,
                                   float eta,
                                   float pt,
                                   float discr) const
{
  ...
  for (unsigned i=0; i<entries.size(); ++i) {
    const BTagCalibration2Reader::TmpEntry &e = entries.at(i);
    if (
