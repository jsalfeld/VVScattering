...
void wzScatteringAnalysisBatch(
 double minMass =  76,
 double maxMass = 106,
 bool applyBtagging = true,
 TString typeLepSel = "defaultTight",
 TString type3rdLepSel = "defaultTight",
 TString outputDirectory = "./",
 bool isChHiggs = false,
int sampleID = 0
 ){
...
infilenamev.push_back(Form("%sTT_TuneCUETP8M1_13TeV-powheg-pythia8+RunIIFall15DR76-PU25nsData2015v1_76X_mcRun2_asymptotic_v12_ext3-v1+AODSIM.root",filesPathMC.Data()));infilecatv.push_back(1);
  ...
  //infilenamev.push_back("/eos/cms/store/user/jsalfeld/ChargedHiggsToWZTo3LNu_M300_13TeV-madgraph-pythia8/mergedHpWZ2016_M300.root");   	infilecatv.push_back(6);
  //infilenamev.push_back("/eos/cms/store/user/jsalfeld/ChargedHiggsToWZTo3LNu_M900_13TeV-madgraph-pythia8/merged_ChargedHiggs_M900.root");   	infilecatv.push_back(6);
...
TH1D* histo_Higgs_CMS_MVALepEffMBoundingAvg    	 = new TH1D( Form("histo_Higgs_%sAvg",effMName)  , Form("histo_Higgs_%sAvg",effMName)  , nBinMVA, xbins); histo_Higgs_CMS_MVALepEffMBoundingAvg  ->Sumw2();
  ...
  for(UInt_t ifile=0; ifile<infilenamev.size(); ifile++) {
    ...
    for (int i=0; i < numberOfEvents; ++i) {
      ...
      //if(infilecatv[ifile] != 6) the_SelBit_tree->GetEntry(i);
      the_SelBit_tree->GetEntry(i);
      if(i%100000==0) printf("event %d out of %d\n",i,(int)the_input_tree->GetEntries());
      
      //if((selBit_ & 0x1<<whichSkim) == 0 && infilecatv[ifile] != 6) continue;
	...
	if(isGenLepton == true) {isGenLep.push_back(1); goodIsGenLep++;}
     ...
     bool passVBFLooseDEtaLess25 = passAllCuts[0] && idJet.size() >= 2 &&
      ...
      bool pass2JetControl = passAllCuts[0] && idJet.size() >= 2 &&
                     (( ( *(TLorentzVector*)(eventJets.p4->At(idJet[0])) ) + ( *(TLorentzVector*)(eventJets.p4->At(idJet[1])) ) )).M() < 500 && 
		   TMath::Abs(((TLorentzVector*)(*eventJets.p4)[idJet[0]])->Eta()-((TLorentzVector*)(*eventJets.p4)[idJet[1]])->Eta()) < 2.5 &&
			(( ( *(TLorentzVector*)(eventJets.p4->At(idJet[0])) ) + ( *(TLorentzVector*)(eventJets.p4->At(idJet[1])) ) )).M() > 100;
