...
void wzScatteringAnalysisBatchSyncImproved(
 double minMass =  76,
 double maxMass = 106,
...
//  cout<<"sample name: "	<<infilenamev[ifile]<<endl;
  ...
  for(UInt_t ifile=0; ifile<infilenamev.size(); ifile++) {
	...
	deltaEtaJJjerDown = TMath::Abs(((TLorentzVector*)(*eventJets.p4)[idJetJERDOWN[0]])->Eta()-((TLorentzVector*)(*eventJets.p4)[idJetJERDOWN[1]])->Eta());
     ...
     bool passVBFtest	 	= false;
     bool passVBFtestJESUP 	= false;
     bool passVBFtestJESDOWN	= false;
	...
	passVBFtest 		= passAllCuts[0] 		&& idJet.size() >= 2 		&& dijet.M() 		>500.	&& dijetDeltaEta > 5.	&& zep3l<2.5;     
	...
	passVBFtestJESUP 	= passAllCutsJESUP 		&& idJetJESUP.size()   >= 2 	&& dijetjesUp.M()	>500.	&& deltaEtaJJjesUp > 5.		&& zep3l < 2.5;
      	      }
     if(idJetJESDOWN.size()>=2){ 
	...
	passVBFtestJESDOWN      = passAllCutsJESDOWN 		&& idJetJESDOWN.size()   >= 2 	&& dijetjesDown.M()	>500.	&& deltaEtaJJjesDown > 5.	&& zep3l < 2.5;
      	      }
   ...
   if(theCategory == 0 && passAllCuts[0]){	  
	...
	else if(thePlot == 41 && passVBFtestJESUP)                        		 	{makePlot = true;theVar = (double)( (TLorentzVector*)(eventJets.p4->At(idJet[0])) )->Pt();}
        else if(thePlot == 42 && passVBFtestJESUP)                         			{makePlot = true;theVar = (double)( (TLorentzVector*)(eventJets.p4->At(idJet[1])) )->Pt();}
        else if(thePlot == 43 && passVBFtestJESUP)                        		 	{makePlot = true;theVar = (double)( (TLorentzVector*)(eventJets.p4->At(idJet[0])) )->Eta();}
        else if(thePlot == 44 && passVBFtestJESUP)                         			{makePlot = true;theVar = (double)( (TLorentzVector*)(eventJets.p4->At(idJet[1])) )->Eta();}
        else if(thePlot == 45 && passVBFtestJESUP)                         		{makePlot = true;theVar = (double) TMath::Min((( ( *(TLorentzVector*)(eventJets.p4->At(idJetJESUP[0])) ) + ( *(TLorentzVector*)(eventJets.p4->At(idJetJESUP[1])) ) )).M(),1900.999);}
	...
	else if(thePlot == 60 && passVBFzep)                            		{makePlot = true;theVar = (double)type3l;}
	...
	if(passVBFLoose){     		MVAVar 			= (double)TMath::Min(superVar(dijet.M(),	dijetDeltaEta,	zp4,	wp4 , zep3l,	isHiggs ),	bound);}
	if(passVBFLooseJESDOWN)	 	MVAVarJESDOWN 		= (double)TMath::Min(superVar(dijetjesDown.M()	,deltaEtaJJjesDown,	zp4,	wp4JESDOWN	,zep3lJesDown, isHiggs)		,bound);
	if(passVBFLooseJESUP)		MVAVarJESUP    		= (double)TMath::Min(superVar(dijetjesUp.M()	,deltaEtaJJjesUp,	zp4,	wp4JESUP	,zep3lJesUp,	isHiggs)	,bound);
	if(passVBFLooseJERDOWN) 	MVAVarJERDOWN 		= (double)TMath::Min(superVar(dijetjerDown.M()	,deltaEtaJJjerDown,	zp4,	wp4JERDOWN	,zep3lJerDown,	isHiggs)	,bound);
	if(passVBFLooseJERUP)		MVAVarJERUP    		= (double)TMath::Min(superVar(dijetjerUp.M()	,deltaEtaJJjerUp,	zp4,	wp4JESUP	,zep3lJerUp,	isHiggs)	,bound);
