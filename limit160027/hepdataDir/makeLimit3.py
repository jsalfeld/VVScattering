...
gmXsecP=array('d', [1735, 979.4, 604.1, 395.7, 270.2, 190.6, 137.8, 101.5, 75.72,36.1,12.1] )
gmXsecM=array('d', [1130, 606.6, 358.4, 225.6, 149., 101.8, 71.47, 51.22, 37.19, 6.9, 1.9])
...
for s in range(2,13):
	...
	if i == 12:
		s=20
	f=ROOT.TFile.Open("higgsCombineTest.Asymptotic.mH"+str(s)+"00.root")
	tree=f.Get("limit")
	tree.GetEntry(2)
	print tree.limit
	median=tree.limit*a
        limit1.append(tree.limit*a)
	limitMD.append(math.sqrt(tree.limit*a/(gmXsecM[i-2]+gmXsecP[i-2])))
        tree.GetEntry(3)
	print tree.limit
        limit168up.append(abs(tree.limit*a-median))
	#limitMD68up.append(math.sqrt(abs(tree.limit*a-median)/(gmXsecM[i-2]+gmXsecP[i-2])))
	limitMD68up.append(abs(math.sqrt(tree.limit*a)-math.sqrt(median))/math.sqrt((gmXsecM[i-2]+gmXsecP[i-2])))
	tree.GetEntry(1)
	print tree.limit
        limit168down.append(abs(tree.limit*a-median))
	#limitMD68down.append(math.sqrt(abs(tree.limit*a-median)/(gmXsecM[i-2]+gmXsecP[i-2])))
	limitMD68down.append(abs(math.sqrt(tree.limit*a)-math.sqrt(median))/math.sqrt((gmXsecM[i-2]+gmXsecP[i-2])))
        tree.GetEntry(4)
	print tree.limit
        limit195up.append(abs(tree.limit*a-median))
	#limitMD95up.append(math.sqrt(abs(tree.limit*a-median)/(gmXsecM[i-2]+gmXsecP[i-2])))
	limitMD95up.append(abs(math.sqrt(tree.limit*a)-math.sqrt(median))/math.sqrt((gmXsecM[i-2]+gmXsecP[i-2])))
	tree.GetEntry(0)
	print tree.limit
        limit195down.append(abs(tree.limit*a-median))
	#limitMD95down.append(math.sqrt(abs(tree.limit*a-median)/(gmXsecM[i-2]+gmXsecP[i-2])))
	limitMD95down.append(abs(math.sqrt(tree.limit*a)-math.sqrt(median))/math.sqrt((gmXsecM[i-2]+gmXsecP[i-2])))
	tree.GetEntry(5)
	limitObserved.append(tree.limit*a)
	limitObservedMD.append(math.sqrt(tree.limit*a/(gmXsecM[i-2]+gmXsecP[i-2])))
...
c1.SetGrid()
graph_limit1=ROOT.TGraph(11,mass,limit1)
...
graph_limit195=ROOT.TGraphAsymmErrors(11,mass,limit1,massErr,massErr,limit195down,limit195up)
graph_limit195.SetLineColor(kBlack)
graph_limit195.SetLineWidth(1)
graph_limit195.SetFillColor(kGreen)
graph_limit195.Draw("3")

graph_limit168=ROOT.TGraphAsymmErrors(11,mass,limit1,massErr,massErr,limit168down,limit168up)
...
c2.SetGrid()
graph_limitGM=ROOT.TGraph(9,mass,limitMD)
...
graph_limitGM95=ROOT.TGraphAsymmErrors(11,mass,limitMD,massErr,massErr,limitMD95down,limitMD95up)
...
graph_limitGM68=ROOT.TGraphAsymmErrors(11,mass,limitMD,massErr,massErr,limitMD68down,limitMD68up)
